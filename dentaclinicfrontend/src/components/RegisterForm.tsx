...
const RegisterForm = () => {
    ...
    const Register = async (e: FormEvent<HTMLFormElement>): Promise<void> => {
        e.preventDefault();
        const data = await fetch("https://localhost:7257/api/register", {
            headers: {
            ...
            <form onSubmit={(e) => Register(e)}>
                                ...
                                {index == 0 ?
                                    ...
                                    (
                                        <>
                                            ...
                                            <Input type='date' onChange={(e) => { onBirthDateChange(e); }} />
                                            <FormLabel>Asmens kodas</FormLabel>
                                            <Input type='number' onChange={(e) => { onPersonalNumberChange(e); }} />
                                            <FormLabel>Gyv. adresas</FormLabel>
                                            <Input type='text' onChange={(e) => { onHomeAddressChange(e); }} />
                                            <FormLabel>Tel. numeris</FormLabel>
                                            <Input type='text' onChange={(e) => { onPhoneNumberChange(e); }} />
                                            <FormLabel>Ūgis</FormLabel>
                                            <Input type='number' onChange={(e) => { onHeightChange(e); }} />
                                            <FormLabel>Svoris</FormLabel>
                                            <Input type='number' onChange={(e) => { onWeightChange(e); }} />
                                        </>
                                    )}
                            </Step>
                        ))}
                ...
                <Flex width="100%" justify="flex-end" mt={5}>
                    ...
                    {activeStep === steps.length - 1 ?
                        <Button type='submit' size="sm">Registruotis</Button>
                        :
